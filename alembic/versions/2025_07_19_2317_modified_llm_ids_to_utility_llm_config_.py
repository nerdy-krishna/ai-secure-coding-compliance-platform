...
def upgrade() -> None:
    ...
    op.add_column(
        "scans", sa.Column("reasoning_llm_config_id", sa.UUID(), nullable=True)
    )
    op.execute(
        "ALTER TABLE scans DROP CONSTRAINT IF EXISTS fk_scans_specialized_llm_config_id_llm_configurations"
    )
    op.execute(
        "ALTER TABLE scans DROP CONSTRAINT IF EXISTS fk_scans_main_llm_config_id_llm_configurations"
    )
    ...
    op.create_foreign_key(
        op.f("fk_scans_reasoning_llm_config_id_llm_configurations"),
        "scans",
        "llm_configurations",
...
def downgrade() -> None:
    ...
    op.create_foreign_key(
        "fk_scans_main_llm_config_id_llm_configurations",
        "scans",
        "llm_configurations",
        ["main_llm_config_id"],
        ["id"],
    ...
    op.create_foreign_key(
        "fk_scans_specialized_llm_config_id_llm_configurations",
        "scans",
        "llm_configurations",
    ...
    op.add_column(
        "findings",
        sa.Column(
            "agent_name", sa.VARCHAR(length=100), autoincrement=False, nullable=True
        ),
    )
