"""ASVS RAG corpus — generated from the OWASP ASVS 5.0 CSV.

Guards that `build_asvs_corpus.py` produces a corpus the ASVS scan
...
_BUILD_SCRIPT = _REPO_ROOT / "scripts" / "build_asvs_corpus.py"
...
def test_asvs_corpus_has_requirements():
    ...
    assert {"id", "document", "control_family"} == set(rows[0].keys())
...
def test_asvs_corpus_in_sync_with_source_csv():
    spec = importlib.util.spec_from_file_location("build_asvs", _BUILD_SCRIPT)
