...
Platform-configuration surface for inspecting and changing the enabled-feature set
...
#: system_config key recording the non-platform accounts that the
#: multi_user ON→OFF transition deactivated, so re-enabling restores exactly
#: those (and never an account a platform manager deactivated by hand).
...
class FeatureUpdateRequest(BaseModel):
    ...
    #: disabling ``multi_user`` (which deactivates every non-platform user).
...
async def _active_non_platform_user_ids(db: AsyncSession) -> List[int]:
    """IDs of every active account without the global platform role."""
    result = await db.execute(
        select(db_models.User.id).where(
            db_models.User.is_active.is_(True),
            ~exists(
                select(db_models.RoleAssignment.id).where(
                    db_models.RoleAssignment.user_id == db_models.User.id,
                    db_models.RoleAssignment.tenant_id.is_(None),
                    db_models.RoleAssignment.role_key == PLATFORM_OWNER,
                )
            ),
        )
    )
...
async def _deactivate_non_platform_users(
    db: AsyncSession,
    ...
    *,
    ...
    """Deactivate every non-platform user; record the IDs for later restore.
    ...
    """
    ids = await _active_non_platform_user_ids(db)
    if ids:
        await db.execute(
    ...
    await repo.set_value(
        api_models.SystemConfigurationCreate(
            ...
            value={"user_ids": ids},
            description="Non-platform users deactivated by the multi_user OFF transition.",
            is_secret=False,
            encrypted=False,
        )
    )
...
async def update_features(
    ...
    *deactivated* (data preserved, login blocked). It therefore requires
    ...
    if disabling_multi_user and not request.confirm_destructive:
        affected = len(await _active_non_platform_user_ids(db))
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                f"Disabling multi_user will deactivate {affected} non-platform "
    ...
    if disabling_multi_user:
        deactivated = await _deactivate_non_platform_users(
            db,
            repo,
            actor=_user,
            request=http_request,
        )
        transition_note = f" {len(deactivated)} non-platform account(s) deactivated."
    elif enabling_multi_user:
        reactivated = await _reactivate_transition_users(db, repo)
        transition_note = f" {len(reactivated)} non-platform account(s) reactivated."
